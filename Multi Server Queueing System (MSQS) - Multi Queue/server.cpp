...
Server :: createTraceFile () {
    ...
    trace_ << "<event> <time> <item id> <server1 status> <server2 status> <queue size>" << endl << endl;
}
...
Server :: arrivalHandler () {
    ...
    trace_ << "a\t" << Scheduler::now () << "\t" << temp->id_ << "\t" << status1_ << "\t" << status2_ << "\t" << queue_->length() << endl;

    if (status1 () == 0) {
        // write to the trace file

        status1() = 1;
        trace_ << "s\t" << Scheduler::now () << "\t" << temp->id_ << "\t" << status1_ << "\t" << status2_<< "\t" << queue_->length() << endl;
        itemInService_ = temp;
...
Server :: departureHandler () {
    // write to the trace file
    if (queue_->length() > 0) {
        trace_ << "d\t" << Scheduler::now () << "\t" << itemInService_->id_ << "\t" << status1_<< "\t" << status2_ << "\t" << queue_->length() << endl;
    } else {
    ...
    if (queue_->length() > 0) {
        ...
        totalQueueDelay_ += queueDelay_;
        // write to the trace file
        trace_ << "s\t" << Scheduler::now () << "\t" << itemInService_->id_ << "\t" << status1_ << "\t" << status2_ << "\t" << queue_->length() << endl;
...
void Server::updateStat()
{
    ...
    areaServer() += (durationSinceLastEvent*status1()+durationSinceLastEvent*status2())/2;
    areaSystem() += durationSinceLastEvent*(queue_->length() + status1());
    areaSystem() += durationSinceLastEvent*(queue_->length() + status2());
}
